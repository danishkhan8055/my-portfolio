...
const Nav = () => {
    ...
    return (
        <nav className={`fixed w-[85%]  m-4 mx-24 rounded-full z-50 transition-all duration-500 ${scrolled ? 'backdrop-blur-md  bg-white/80 shadow-sm py-2' : 'backdrop-blur-sm bg-white/30  py-4'}`}>
            <div className="container mx-auto px-4 flex items-center justify-between">
                ...
                >
                    <span className={`text-3xl font-semibold ${scrolled ? 'text-gray-800' : 'text-white'}`}>
                        Code <span className="text-5xl font-bold bg-gradient-to-r from-[#c86e7e] to-[#afeb5b] bg-clip-text text-transparent">DanY</span>
                    </span>
                </motion.div>
                        ...
                        {navItems.map((item) => (
                            <li key={item.id}>
                                <AnchorLink 
                                    href={`#${item.id}`} 
                                    offset={50}
                                    className={`relative no-underline text-lg font-medium transition-colors duration-300 ${select === item.id ? (scrolled ? 'text-gray-900' : 'text-white') : (scrolled ? 'text-gray-700 hover:text-gray-900' : 'text-gray-200 hover:text-white')}`}
                                ...
                                {navItems.map((item) => (
                                    ...
                                    >
                                        <AnchorLink 
                                            href={`#${item.id}`} 
                                            offset={50}
                                            className={`relative no-underline text-xl font-medium ${select === item.id ? 'text-gray-900' : 'text-gray-700'}`}
                                            onClick={() => {
                                                setSelect(item.id);
                                                toggleMenu();
                                            }}
